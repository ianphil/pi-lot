# pi-lot Docs

`pi-lot` ships a Copilot-backed .NET SDK, local OpenAI-compatible proxy,
terminal client, agent loop, and experimental UI.

## Guides

| Guide | Scope |
|---|---|
| [SDK guide](sdk-guide.md) | LlmSdk setup, usage, streaming, and error handling |
| [SDK developer docs](sdk/) | Deeper LlmSdk architecture and feature guides |
| [Agent guide](agent-guide.md) | Tool-calling agent loop built on the SDK |
| [CLI guide](cli-guide.md) | Terminal client commands and examples |
| [Proxy API reference](api-reference.md) | Local OpenAI-compatible HTTP endpoints |
| [Event log guide](event-log-guide.md) | Windows Event Log structure |
| [Compliance tests](how-to-run-compliance-tests.md) | OpenResponses compliance runner |

For the project overview and quick start, see the
[repository README](https://github.com/ianphil/pi-lot).
