...
public sealed class ResponsesEndpointTests : IClassFixture<ResponsesWebApplicationFactory>
{
    ...
    public async Task PostResponses_WithProxyKnobHeaders_ForwardsAllowedOptions()
    {
        ...
        request.Headers.Add("X-LLM-Request-Id", "responses-test");
        request.Headers.Add("X-LLM-Correlation-Id", "responses-correlation");
        request.Headers.Add("X-LLM-Metadata-test", "responses-metadata");
        request.Headers.Add("X-LLM-Timeout-Ms", "60000");
        request.Headers.Add("X-LLM-Max-Retries", "2");
        request.Headers.Add("X-LLM-Max-Retry-Delay-Ms", "1000");
        ...
        Assert.Equal("responses-correlation", _factory.Provider.LastResponsesRequest.CorrelationId);
        Assert.Equal("responses-metadata", Assert.IsType<Dictionary<string, string>>(_factory.Provider.LastResponsesRequest.Metadata)["test"]);
    ...
    public async Task PostResponses_WithMalformedProxyKnobHeader_ReturnsBadRequest()
    {
        ...
        request.Headers.Add("X-LLM-Timeout-Ms", "not-an-int");
    ...
    [Fact]
    public async Task PostResponses_WithMalformedMetadataHeader_ReturnsBadRequest()
    {
        _factory.Provider.ResetCapturedRequests();
        _factory.Provider.Models =
        [
        ...
        request.Headers.Add("X-LLM-Metadata-bad~key", "value");
