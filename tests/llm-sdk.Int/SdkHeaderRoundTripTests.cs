...
public sealed class SdkPerCallKnobTests
{
    ...
    public async Task SendResponsesAsync_WithGitHubRequestAndCorrelationHeaders_ReturnsObservedUpstreamHeaders()
    {
        ...
        var returnedGitHubRequestId = Assert.Single(GetRequiredHeader(result.Headers, "X-GitHub-Request-Id"));
        var returnedRequestId = Assert.Single(GetRequiredHeader(result.Headers, "X-Request-Id"));
        Assert.Equal(200, result.StatusCode);
        Assert.NotEqual(requestId, returnedGitHubRequestId);
        Assert.NotEqual(requestId, returnedRequestId);
        Assert.NotEmpty(returnedGitHubRequestId);
        Assert.NotEmpty(returnedRequestId);
